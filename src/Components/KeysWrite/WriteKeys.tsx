...
function WriteKeys({ onKeyClick, setKeyRef }: WriteKeysProps) {
  ...
  return (
    <div className={styles.keys}>
      <Key onClick={(_, e) => onKeyClick('^32', e)} char={'^32'} ref={setKeyRef('^32')} wide>
        <SpaceSymbol title={l('spaceWord')} name={l('spaceWord')} aria-label={l('spaceWord')} />
      </Key>
      <Key onClick={(_, e) => onKeyClick('^8', e)} char={'^8'} ref={setKeyRef('^8')} wide>
        <BackspaceSymbol

...
const findNotLowerKey = (words) => (
  Object.keys(words).find(k => k !== k.toLowerCase())
);

const findNotUpperValue = (words) => (
  Object.values(words).flat().find(v => v !== v.toUpperCase())
);
...
const findInvalidChars = (words, chars) => (
  Object.values(words).flat().find(v => !(new RegExp(`^[${chars}]+$`)).test(v))
);
...
function verify() {
  for (const lang of langDirs) {
    ...
    try {
      words = JSON.parse(
        fs.readFileSync(`${LANGS_DIRECTORY}/${lang}/${WORDS_FILE}`, 'utf8')
      );
    }
    catch {
    ...
    try {
      chars = JSON.parse(
        fs.readFileSync(`${LANGS_DIRECTORY}/${lang}/${TRANSL_FILE}`, 'utf8')
      ).alphabet + ' ';
    }
    catch {
